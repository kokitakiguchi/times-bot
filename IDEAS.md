...
## 📌 仕様変更の提案: 初回チャンネル作成時は転送せずアナウンスのみ

> ステータス: **未実装（提案のみ）**。実装する場合は feature ブランチで対応し、[README.md](README.md) の「自動登録フロー」も更新する。

### 背景 / 動機

- 現状、ユーザーの初回投稿では (1) `times-<username>` チャンネルを自動作成し、(2) アナウンスを送信し、(3) その初回投稿を転送する、の3つが起きる。
- アナウンス文言（「times チャンネルを作りました！…個人ので好きなことを自由につぶやいてOK…他の人の times も自由に覗きに行けますよ！」）は、**ユーザーが自分の times チャンネルで直接つぶやくスタイル**を案内している。
- そのため初回作成時にソース投稿を転送すると、チャンネル冒頭に「アナウンス + 転送メッセージ」が並んでしまい、案内と噛み合わない。初回は **設定済みのアナウンスのみ** を表示したい。

### 変更後の挙動

| タイミング | 現状 | 変更後 |
| --- | --- | --- |
| 初回（チャンネル新規作成時） | アナウンス + 初回投稿を転送 + DB保存 | **アナウンスのみ送信**。トリガー投稿は転送・DB保存しない |
| 2回目以降（チャンネル既存） | 転送 + DB保存 | 変更なし（従来どおり転送） |

- ユーザー登録（`users` テーブルの upsert）は初回でも従来どおり実施する。

### 実装メモ

- 対象: [src/bot.ts](src/bot.ts) の `handleIncomingMessage`
- `channelJustCreated === true` の分岐でアナウンス送信後に `return` し、`destinationChannel.send(decision.payload)` と `store.saveMessage` をスキップする。
- **注意**: `messages.forwarded_message_id` は `NOT NULL` 制約（[src/db.ts](src/db.ts)）。転送しない初回投稿は `messages` に保存できないため、初回投稿は「登録トリガー」として扱い `messages` には記録しない（本文は残らない）。

### 検討事項 / トレードオフ

- 初回投稿の本文が DB に残らない点を許容するか。許容しない場合は `forwarded_message_id` を nullable にするマイグレーションが別途必要（スキーマ変更 = MAJOR 候補）。
- 「初回だけ転送されない」挙動はユーザーから見て分かりにくい可能性。アナウンス文で「ここで自由につぶやいてOK」と times 側での投稿導線を強調すれば自然に見える。
